...
# ---------------------- Параметры ----------------------
xlsx_path = r"C:\Документы\ДИПЛОМ\Химченко_данные\Термокосы\st3.xlsx"
output_dir = r"C:\Документы\ДИПЛОМ\Химченко_данные\Термокосы\st3-results"
...
def plot_scheme(depths, outpath=None, show=True):
    ...
    ax.set_title('Схема теромокосы №3 с датчиками')
    fig.tight_layout()
    if outpath:
...
def plot_temp_field(times_1d, depths, temps, outpath=None, show=True):
    ...
    ax.set_ylabel('Глубина, м')
    ax.set_xlabel('Время')
    ax.set_title('Временная изменчивость температуры на st3')
    ax.invert_yaxis()
...
def plot_isotherms_all(times_1d, depths_iso, isotherms, depths, outpath=None, show=True):
    ...
    ax.set_ylabel('Глубина, м')
    ax.set_xlabel('Время')
    ax.set_title('График изотерм на st3')
    ax.grid(True)
...
def plot_isotherms_separately(times_1d, depths_iso, isotherms, depths, colors=None, outdir=None, show=True):
    ...
    for ii, iso in enumerate(isotherms):
        ...
        ax.set_ylabel('Глубина, м')
        ax.set_xlabel('Время')
        ax.set_title(f'Глубинный профиль изотермы {iso}°C на st3')
        ax.grid(True)
        ax.xaxis.set_major_formatter(mdates.DateFormatter('%d-%b-%Y %H:%M:%S'))
        fig.autofmt_xdate()
        fig.tight_layout()
...
if st1 is None:
    ...
    plt.title(f"Спектр изотермы {chosen_temp}°C на st3")
