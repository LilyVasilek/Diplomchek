# -*- coding: utf-8 -*-
# =========================================================
# 13. АНАЛИЗ ВОЛН НА ВЫБРАННОЙ ИЗОТЕРМЕ (wave_iso, самый длинный непрерывный участок)
#     Статистика короткопериодных волн + PSD повторяемых изотерм
# =========================================================
t_win = t_segment
z_wave_win = z_segment

print(
    f"\nИнтервал анализа волн (изотерма {wave_iso:.1f} °C): "
    f"{t_win[0].strftime('%d.%m.%Y %H:%M')} — "
    f"{t_win[-1].strftime('%d.%m.%Y %H:%M')}"
)
...
def _chi2_partition(x, frozen, k0, n, min_exp=5.0):
    """Группировка для χ²: k₀ равных интервалов → объединение при ΣE_i ≥ min_exp."""
    counts0, edges0, k0_used = _histogram_sturges(x, k0)
    exp0 = _expected_bin_counts_raw(frozen, edges0, n)
    n_obs, edges, exp = _combine_bins_mpy(counts0, edges0, exp0, min_exp=min_exp)
    p = exp / n if n > 0 and exp is not None else None
    widths = np.diff(edges) if edges is not None and len(edges) > 1 else None
    h0 = float((edges0[-1] - edges0[0]) / k0_used) if k0_used > 0 else np.nan
    return {
        "counts": n_obs,
        "expected": exp,
        "p": p,
...
def chi2_gof_test(values, dist, fit_kw, k_init, alpha=0.05, min_exp=5.0):
    ...
    k₀ от Стерджесса; при ν ≤ 0 k₀ увеличивается, пока ν = k − m − 1 ≥ 1.
    Объединение интервалов как в m.py (ΣE_i >= min_exp).
    """
    x = np.asarray(values, dtype=float)
    x = x[np.isfinite(x) & (x > 0)]
    if x.size < 5:
    ...
    n = int(x.size)
    k0_start = max(1, int(k_init) if k_init is not None else _sturges_k(n))
    k0_min_for_df = m + 2  # ν ≥ 1 ⇔ k ≥ m + 2
    k0_max = min(max(k0_start + 30, k0_min_for_df + 8), max(20, n // 3))

    def _pack(part, df, chi2=np.nan, crit=np.nan, p_value=np.nan, reject=None):
        k = part["k_merged"]
        return {
            "chi2": chi2,
            "df": df,
    ...
    best_invalid = None
    for k0_try in range(k0_start, k0_max + 1):
        part = _chi2_partition(x, frozen, k0_try, n, min_exp=min_exp)
        exp = part["expected"]
        k = part["k_merged"]
        df = k - m - 1
        valid = (
        ...
        if not valid:
            cand = _pack(part, df)
            if best_invalid is None or cand["df"] > best_invalid["df"]:
                best_invalid = cand
            continue

        chi2 = float(np.sum((part["counts"] - exp) ** 2 / exp))
        crit = float(stats.chi2.ppf(1.0 - alpha, df))
        p_value = float(stats.chi2.sf(chi2, df))
        return _pack(part, df, chi2, crit, p_value, chi2 > crit)
...
def _print_chi2_table(result, var_name, unit):
    ...
    k0_note = ""
    if result.get("k0_start") and result["k0"] > result["k0_start"]:
        k0_note = f" (увеличено с {result['k0_start']} для ν ≥ 1)"
    print(
        f"    n={result['n']}, k₀={result['k0']}{k0_note} (равные Δ₀"
    ...
    if result["reject"] is None:
        k_m = result.get("k_merged", result["k"])
        m_par = result["m"]
        nu = result["df"]
        k0s = result.get("k0_start", result["k0"])
        print(
            f"    Тест не применим: ν = k − m − 1 = {k_m} − {m_par} − 1 = {nu} "
            f"(нужно ν ≥ 1 при E_i ≥ 5)."
        )
        if nu <= 0 and result.get("k0", k0s) > k0s:
            print(
                f"    k₀ увеличен с {k0s} до {result['k0']}, "
                f"после объединения k = {k_m} — недостаточно интервалов."
            )
        elif nu <= 0:
            print(
                f"    При k₀ = {result['k0']} после объединения остаётся k = {k_m} "
                f"интервалов — для данного закона χ² не определяется."
            )
    elif result["reject"]:
...
def _plot_hist_plain(ax, values, edges, color, xlabel, title):
    ax.hist(
        values,
        bins=edges,
        density=True,
...
def _mean_ci_table(values, var_name, unit, k_sturges, alpha=0.05, edges=None):
    """Таблица ДИ для M(Y) в центрах интервалов гистограммы (Стерджесс)."""
    values = np.asarray(values, dtype=float)
    values = values[np.isfinite(values)]
    n_waves = int(values.size)
    if n_waves < 3:
        return None
    if edges is None:
...
def print_wave_mean_ci_report(table_h, table_t, wave_iso=None):
    ...
    print("=" * 72)
    if wave_iso is not None:
        print(f"Изотерма (те же волны, что на гистограммах H и T): {wave_iso:.1f} °C")
    print(
        "Формула (95%):  y_p - t*s(y_p) <= M(Y) <= y_p + t*s(y_p),\n"
        "          s(y_p)^2 = (s^2/n) * (1 + (x_p - x_mean)^2 / Var(x)),\n"
        "  n — число интервалов с данными; N — всего волн на изотерме;\n"
        "  регрессия по (центр интервала, среднее волн в интервале)."
    )
    for pack in (table_h, table_t):
        ...
        print(f"  Всего волн на изотерме N = {pack['n_waves']}")
        print(
            f"  Интервалов в регрессии n = {ci['n']}, "
            f"степени свободы nu = {ci['df']}, k (Стерджесс) = {pack['k']}"
        )
        print(
...
def plot_wave_mean_ci_figure(
    ...
    """Доверительные интервалы для H и T — те же волны и бины, что на гистограммах."""
    base_dir = os.path.dirname(os.path.abspath(__file__))
    out_path = out_file if os.path.isabs(out_file) else os.path.join(base_dir, out_file)
    txt_path = os.path.splitext(out_path)[0] + ".txt"
    n_waves = int(n_waves) if n_waves is not None else len(H)

    table_h = _mean_ci_table(
        H, "Высота волн H", "м", k_sturges, alpha=alpha, edges=edges_h,
    )
    table_t = _mean_ci_table(
        Tm, "Период волн T", "мин", k_sturges, alpha=alpha, edges=edges_t,
    )

    iso_line = (
        f"изотерма {wave_iso:.1f} °C, N = {n_waves} волн"
        if wave_iso is not None
        else title_suffix
    )
    fig, (ax_h, ax_t) = plt.subplots(1, 2, figsize=(14, 5.5))
    fig.suptitle(
        f"Доверительные интервалы 95% — волны на {iso_line}\n"
        "Интервалы Стерджесса совпадают с гистограммами H и T",
        fontsize=12,
        y=1.02,
    )
    _plot_mean_ci_panel(
    ...
    buf_lines = [
        ...
        f"Всего волн N = {n_waves}",
        "",
        "Формула: y_p - t*s(y_p) <= M(Y) <= y_p + t*s(y_p)",
        "         s(y_p)^2 = (s^2/n) * (1 + (x_p - x_mean)^2 / Var(x))",
        "  n — число интервалов регрессии; N — число волн на изотерме.",
    ]
    for pack in (table_h, table_t):
        if pack is None:
            continue
        ...
        buf_lines.append(
            f"alpha={pack['alpha']}, N_волн={pack['n_waves']}, "
            f"n_интерв={ci['n']}, nu={ci['df']}, k={pack['k']}"
        )
...
def plot_wave_histograms(
    H, Tm, title_suffix, out_file, k_sturges=None, edges_h=None, edges_t=None,
):
    """Гистограммы H и T (Стерджесс), без KDE."""
    base_dir = os.path.dirname(os.path.abspath(__file__))
    out_path = out_file if os.path.isabs(out_file) else os.path.join(base_dir, out_file)
    ...
    print(f"\nГистограммы (без KDE): {out_path}")
    plt.close(fig_hist)


def plot_wave_histograms_kde(
    H, Tm, title_suffix, out_file, k_sturges=None, edges_h=None, edges_t=None,
):
...
def plot_wave_histograms_fitted(H, Tm, title_suffix, out_file, k_sturges=None):
    ...
    fig.savefig(out_path, dpi=200, bbox_inches="tight")
    print(f"Гистограммы (подогнанные законы): {out_path}")
    plt.close(fig)


# Иерархическая кластеризация: ρ — Евклид (H, T в м и мин), центр — среднее, K — «локоть».
HIER_LINKAGE_METHOD = "centroid"
WAVE_ELBOW_K_MIN = 2  # минимум K при выборе «локтя» для кластеризации
WAVE_ELBOW_K_MAX = 10  # максимум K при выборе «локтя» (не путать с n волн на графике)
WAVE_CLUSTER_MIN_N = 10  # min(n_j) в кластере; K с локтя не меняется
...
def _hierarchical_labels(x, n_clusters, method=HIER_LINKAGE_METHOD):
    """
    Иерархическая кластеризация по ρ между центрами кластеров (linkage centroid).
    Точки: (H, м; T, мин), без нормировки.
    """
    from scipy.cluster.hierarchy import linkage, fcluster

    x = np.asarray(x, dtype=float)
    z_link = linkage(x, method=method)
    lab = fcluster(z_link, t=n_clusters, criterion="maxclust")
    return (lab - 1).astype(int), z_link


def _min_cluster_size(labels):
    labels = np.asarray(labels, dtype=int)
    sizes = [int(np.sum(labels == c)) for c in np.unique(labels)]
    return min(sizes) if sizes else 0


def _cluster_sizes_map(labels, k_total):
    labels = np.asarray(labels, dtype=int)
    return {c: int(np.sum(labels == c)) for c in range(int(k_total))}


def _enforce_min_cluster_size(x, labels, k_total, min_n=WAVE_CLUSTER_MIN_N):
    """
    При фиксированном K:
    1) волны из кластеров с n < min_n → ближайший другой кластер;
    2) пустые кластеры пополняются из самого крупного (n ≥ 2·min_n).
    """
    x = np.asarray(x, dtype=float)
    ...
    for _ in range(max_iter):
        changed = False
        sizes = _cluster_sizes_map(labels, k_total)

        for c in range(k_total):
            if sizes[c] == 0 or sizes[c] >= min_n:
                continue
            others = [t for t in range(k_total) if t != c and sizes[t] > 0]
            centers = {}
            for t in others:
                mask = labels == t
                if np.any(mask):
                    centers[t] = _cluster_centroid(x[mask])
            if not centers:
                continue
            for i in np.where(labels == c)[0]:
                t_best = min(centers, key=lambda t: _euclidean_dist(x[i], centers[t]))
                if labels[i] != t_best:
                    labels[i] = t_best
                    changed = True
        ...
        for c in range(k_total):
            if sizes[c] != 0:
                continue
            donor = max(
                (t for t in range(k_total) if t != c and sizes[t] >= 2 * min_n),
                key=lambda t: sizes[t],
                default=None,
            )
            if donor is None:
                donor = max(
                    (t for t in range(k_total) if t != c and sizes[t] > min_n),
                    key=lambda t: sizes[t],
                    default=None,
                )
            if donor is None:
                continue
            idx_donor = np.where(labels == donor)[0]
            n_take = min(min_n, len(idx_donor) - min_n)
            if n_take < min_n:
                continue
            cent_donor = _cluster_centroid(x[idx_donor])
            dists = np.array([_euclidean_dist(x[i], cent_donor) for i in idx_donor])
            farthest = idx_donor[np.argsort(dists)[-n_take:]]
            labels[farthest] = c
            changed = True
...
def _elbow_k_from_curve(k_vals, y_vals):
    """
    K после наибольшего относительного уменьшения ρ при переходе K−1 → K (метод «локтя»).
    """
    k = np.asarray(k_vals, dtype=int)
    y = np.asarray(y_vals, dtype=float)
    if k.size < 2:
        return int(k[0]) if k.size else WAVE_ELBOW_K_MIN
    rel_drop = (y[:-1] - y[1:]) / (y[:-1] + 1e-12)
    return int(k[int(np.argmax(rel_drop)) + 1])


def _merge_distance_at_k(z_link, n_pts, k):
    """ρ слияния при переходе к k кластерам; при k = n каждый элемент — отдельный кластер (ρ = 0)."""
    if k >= n_pts:
        return 0.0
    return float(z_link[n_pts - k - 1, 2])


def _merge_step_xticks(n, k_elbow, max_ticks=9):
    """Метки оси K без наложений: n, 1, локоть и равномерный шаг."""
    must = {1, n, int(k_elbow)}
    if n <= max_ticks:
        return sorted(must | set(range(1, n + 1)), reverse=True)
    step = max(1, int(np.ceil((n - 2) / max(1, max_ticks - len(must)))))
    extra = set(range(1, n + 1, step))
    ticks = sorted(must | extra, reverse=True)
    if len(ticks) > max_ticks + 2:
        step = max(1, int(np.ceil(n / max_ticks)))
        ticks = sorted(must | set(range(1, n + 1, step)), reverse=True)
    return ticks


def plot_wave_cluster_elbow(
    heights_m,
    periods_min,
    title_suffix,
    out_file,
    method=HIER_LINKAGE_METHOD,
):
    """
    Метод «локтя»: ρ на каждом шаге слияния, K от n (каждый элемент — кластер) до 1.
    """
    from scipy.cluster.hierarchy import linkage

    n = len(heights_m)
    if n < 4:
        print(f"  Метод «локтя»: n={n} < 4, график не строится.")
        return None

    x = np.column_stack([heights_m, periods_min])
    z_link = linkage(x, method=method)
    k_pick_lo = max(WAVE_ELBOW_K_MIN, 2)
    if n < k_pick_lo:
        print(f"  Метод «локтя»: недостаточно волн (n={n}).")
        return None

    k_plot = list(range(n, 0, -1))
    merge_plot = [_merge_distance_at_k(z_link, n, k) for k in k_plot]

    # K для кластеризации — только 2…10 (локоть); на графике K=n — лишь старт слияний.
    k_hi_pick = min(WAVE_ELBOW_K_MAX, n - 1)
    k_pick = list(range(k_pick_lo, k_hi_pick + 1))
    dist_pick = [_merge_distance_at_k(z_link, n, k) for k in k_pick]
    k_elbow = _elbow_k_from_curve(k_pick, dist_pick)
    rejected_ks = set()
    for k in k_pick:
        lab, _ = _hierarchical_labels(x, k)
        if _min_cluster_size(lab) < WAVE_CLUSTER_MIN_N:
            rejected_ks.add(k)
    y_mark = _merge_distance_at_k(z_link, n, k_elbow)
    base_dir = os.path.dirname(os.path.abspath(__file__))

    fig_w = float(np.clip(8.0 + n / 55.0, 8.0, 13.0))
    ...
    ax.set_xlim(n + 0.02 * n, 0.98)
    ax.set_xlabel("Число кластеров K")
    ax.set_ylabel("Расстояние между кластерами при слиянии ρ")
    ax.set_title(
        "Расстояние между кластерами на каждом шаге слияния\n"
        f"(H — м, T — мин; {title_suffix})",
        fontsize=10,
        pad=10,
    )
    xticks = _merge_step_xticks(n, k_elbow)
    ...
    ax.margins(y=0.08)
    fig.subplots_adjust(left=0.10, right=0.97, top=0.90, bottom=0.16 if len(xticks) > 8 else 0.12)
    out_path = os.path.join(base_dir, out_file)
    fig.savefig(out_path, dpi=150, bbox_inches="tight", pad_inches=0.15)
    plt.close(fig)

    print(f"\nМетод «локтя» ({method}, n = {n}, H — м, T — мин):")
    print("     K    ρ слияния")
    for k, d in zip(k_pick, dist_pick):
        if k == k_elbow:
            mark = "  ←"
        elif k in rejected_ks:
            mark = "  ×"
        else:
            mark = ""
        print(f"  {k:3d}  {d:12.4f}{mark}")
    print(f"  K = {k_elbow}  (× — min n < {WAVE_CLUSTER_MIN_N} до переназначения)")
    print(f"  График: {out_path}")
    return k_elbow, k_plot, merge_plot
...
def print_cluster_summary_table(df_clusters, alpha=0.05, out_txt=None, k_total=None):
...
def cluster_and_plot_waves(
    ...
    """Иерархическая кластеризация волн (H, T); K — с графика «локтя»."""
    n = len(heights_m)
    if n < n_clusters:
        print(
    ...
    x = np.column_stack([heights_m, periods_min])
    k = int(np.clip(n_clusters, WAVE_ELBOW_K_MIN, min(WAVE_ELBOW_K_MAX, n - 1)))
    labels, _z_link = _hierarchical_labels(x, k)
    labels = _enforce_min_cluster_size(x, labels, k)
    ...
    print(
        ...
        f"(ρ евклидова, центр — среднее, связь: {HIER_LINKAGE_METHOD}, K={k}, H — м, T — мин):"
    )
    for c in range(k):
        mask = labels == c
        if not np.any(mask):
    ...
    ax_c.set_title(
        f"Кластеры волн по амплитуде и периоду (K={k})\n{title_suffix}",
        fontsize=10,
    )
    ...
    print_cluster_summary_table(df_clusters, alpha=0.05, out_txt=txt_path, k_total=k)
...
def build_waves_on_isotherm(
    ...
    """Волны на самом длинном непрерывном участке изотермы wave_iso_c."""
    z_shift = np.asarray(z_seg, dtype=float) - np.nanmean(z_seg)
    waves = detect_waves(
        z_shift,
        dt_seconds=dt_sec,
    ...
    rows = []
    for n, (i0, i1, _imax, h_wave, period_min) in enumerate(waves, start=1):
        rows.append({
            "№": n,
            "Начало": time_seg[i0].strftime("%d.%m %H:%M:%S"),
            "Конец": time_seg[i1].strftime("%d.%m %H:%M:%S"),
            "Высота H, м": h_wave,
            "Период T, мин": period_min,
        })
    df = pd.DataFrame(rows)
    print(
        f"\nВолны для гистограмм и ДИ: изотерма {wave_iso_c:.1f} °C, "
        f"h >= {min_height_m} м, T >= {min_period_min} мин — найдено {len(waves)}"
    )
    return df, waves


def run_wave_histogram_and_ci_analysis(df_waves, wave_iso_c, wave_iso_tag):
    """Статистика, ДИ, гистограммы и кластеры — одна выборка волн на wave_iso."""
    H = df_waves["Высота H, м"].to_numpy(dtype=float)
    Tm = df_waves["Период T, мин"].to_numpy(dtype=float)
    n_waves = len(df_waves)
    k_st = max(1, int(np.ceil(1 + np.log2(n_waves))))
    edges_h, _ = _sturges_edges(H, k_st)
    edges_t, _ = _sturges_edges(Tm, k_st)
    wave_title = f"изотерма {wave_iso_c:.1f} °C, N = {n_waves} волн"
    ...
    print_wave_mean_ci_report(ci_tables[0], ci_tables[1], wave_iso=wave_iso_c)
    print_wave_chi2_tests(H, Tm, k_sturges=k_st, alpha=0.05)
    plot_wave_histograms(
        H,
        Tm,
        title_suffix=wave_title,
        out_file=f"waves_{wave_iso_tag}_hist.png",
        k_sturges=k_st,
        edges_h=edges_h,
        edges_t=edges_t,
    )
    ...
    elbow_result = plot_wave_cluster_elbow(
        H,
        Tm,
        title_suffix=wave_title,
        out_file=f"waves_{wave_iso_tag}_sse_vs_k.png",
    )
    k_clusters = elbow_result[0] if elbow_result is not None else None
    cluster_result = None
...
df_waves_iso, selected_waves = build_waves_on_isotherm(
    z_wave_win, t_win, wave_iso, dt,
)


# Окно короткого участка part4: фиксировано 20.06 23:30 — 21.06 01:00 (part2).
if "SHORT_SURGE_START" in globals():
    print(
        f"Короткий участок (part4): {SHORT_SURGE_START:%d.%m.%Y %H:%M} — "
        f"{SHORT_SURGE_END:%d.%m.%Y %H:%M}"
    )

if len(df_waves_iso) > 0:
    print(f"\nТаблица волн на изотерме {wave_iso:.1f} °C (гистограммы и ДИ):")
    print(
        df_waves_iso.to_string(
            index=False, justify="center", float_format=lambda x: f"{x:.2f}",
        )
    )

    cluster_result = run_wave_histogram_and_ci_analysis(df_waves_iso, wave_iso, wave_iso_tag)
    ...
    print(
        f"Недостаточно волн на изотерме {wave_iso:.1f} °C "
        f"для гистограмм и доверительных интервалов."
    )
...
if len(top_isos) > 0:
    ...
    out_psd = os.path.join(
        os.path.dirname(os.path.abspath(__file__)),
        "top_repeated_isotherms_psd_full_segment.png",
    )
    fig_psd.savefig(out_psd, dpi=200)
    print(f"График PSD: {out_psd}")
    plt.close(fig_psd)
else:
