...
def _continuous_segment_for_iso(T_iso):
...
print(f"\nЧисло волн (H >= {WAVE_MIN_HEIGHT_M} м, T >= 3 мин) на своём непрерывном участке:")
_compare_set = list(dict.fromkeys(list(iso_values) + [float(best_iso)]))
for T_try in _compare_set:
    seg_try = _continuous_segment_for_iso(T_try)
    if seg_try is None:
        print(f"  {T_try:.1f}°C: нет непрерывного ряда")
        continue
    n_w = _count_waves_on_segment(seg_try[2], dt)
...
_wave_seg = _continuous_segment_for_iso(wave_iso)
...
z_mean_wave = np.nanmean(z_segment)
